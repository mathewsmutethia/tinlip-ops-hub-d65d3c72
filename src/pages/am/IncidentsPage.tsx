  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-4">
        {statuses.map(s => (
          <button key={s} onClick={() => setStatusFilter(s)} className={cn('px-3 py-1.5 text-xs rounded-md font-medium', statusFilter === s ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/80')}>
            {s}
          </button>
        ))}
      </div>
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">No incidents found</td></tr>
            )}
          </tbody>
        </table>
      </div>
