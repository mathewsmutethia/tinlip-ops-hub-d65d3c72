...
type Incident = Pick<Tables<'incidents'>, 'id' | 'claim_code' | 'type' | 'location' | 'status' | 'created_at'> & {
  clients: Pick<Tables<'clients'>, 'name'> | null;
  vehicles: Pick<Tables<'vehicles'>, 'registration'> | null;
};
            ...
            {!loading && filtered.map(i => (
              <tr key={i.id} className="border-b hover:bg-table-hover cursor-pointer">
                <td className="px-4 py-3 font-mono font-medium">
                  <Link to={`/incidents/${i.id}`} className="text-primary hover:underline">
